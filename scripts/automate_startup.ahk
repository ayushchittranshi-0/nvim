...
if (PCTypeValue = "work")
{
   ...
   Sleep 4000
   Send !j
   Run, cmd.exe /C wt --maximized nt -p "Ubuntu"  --title "jpgw6"
   Sleep 2000
   Send !j
   Run, cmd.exe /C wt --maximized nt -p "Ubuntu"  --title "jntw7" 
   Sleep 2000
   Send !k
   Send !k
   Send !k
...
{
    Run, cmd.exe /C start firefox "http://localhost:3000/"
    Run, cmd.exe /C start chrome "http://localhost:3000/" --new-window --window-name="fvf1" 
}